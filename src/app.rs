...
pub struct App {
    ...
    pub app_logs: std::collections::HashMap<String, Vec<String>>,
    ...
    pub log_tx: tokio::sync::mpsc::UnboundedSender<(String, String)>,
}

impl App {
    pub fn new(log_tx: tokio::sync::mpsc::UnboundedSender<(String, String)>) -> App {
        ...
        App {
            ...
            app_logs: std::collections::HashMap::new(),
    ...
    pub async fn refresh_data(&mut self) -> Result<()> {
        ...
        if let Ok(running) = crate::flatpak::get_running_apps().await {
            self.running_ids = running.into_iter().collect();
        }
    ...
    pub fn launch_selected(&mut self) {
        ...
        if let Some(i) = self.table_state.selected() {
            if i < list.len() {
                ...
                let flag = self.background_task_completed.clone();
                self.status_message = Some(format!("Launching {}...", id));
                tokio::spawn(async move {
                    let res = crate::flatpak::launch_app(&id, tx).await.map_err(|e| e.to_string());
                    if let Ok(mut g) = flag.lock() { *g = Some(res); }
                });
            }
        }
    ...
    pub fn stop_selected(&mut self) {
        ...
        if let Some(i) = self.table_state.selected() {
            if i < list.len() {
                let id = list[i].application_id.clone();
                if !self.running_ids.contains(&id) { return; }
                let flag = self.background_task_completed.clone();
                self.status_message = Some(format!("Stopping {}...", id));
                tokio::spawn(async move {
                    let res = crate::flatpak::stop_app(&id).await.map_err(|e| e.to_string());
                    if let Ok(mut g) = flag.lock() { *g = Some(res); }
                });
            }
        }
    ...
    pub fn toggle_logs_view(&mut self) {
        if self.view_mode == ViewMode::Logs {
            ...
            // Ensure we have an entry for logs
            self.app_logs.entry(id).or_insert_with(Vec::new);
        }
    }
    ...
    pub fn handle_normal_key(&mut self, code: KeyCode) -> Option<bool> {
        ...
        match code {
            ...
            KeyCode::Char('L') => self.toggle_logs_view(),
            _ => {}
        }
        None
    }
