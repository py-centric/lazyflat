...
pub async fn launch_app(application_id: &str, log_tx: tokio::sync::mpsc::UnboundedSender<(String, String)>) -> Result<()> {
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = reader.next_line().await {
            let _ = tx_stdout.send((id_stdout.clone(), line));
        }
    });

    let id_stderr = id.clone();
    let tx_stderr = log_tx;
    tokio::spawn(async move {
        let mut reader = tokio::io::BufReader::new(stderr).lines();
        while let Ok(Some(line)) = reader.next_line().await {
            let _ = tx_stderr.send((id_stderr.clone(), line));
        }
    });
...
pub async fn stop_app(application_id: &str) -> Result<()> {
    let output = Command::new("flatpak")
        .arg("kill")
        .arg(application_id)
        .output()
    ...
    if !output.status.success() {
        anyhow::bail!("Failed to kill application {}", application_id);
    }
    Ok(())
}

pub async fn get_running_apps() -> Result<Vec<String>> {
    let output = Command::new("flatpak")
        .arg("ps")
    ...
    for line in stdout.lines() {
        let line = line.trim();
        if !line.is_empty() && line != "Application" {
            running.push(line.to_string());
        }
    }
    Ok(running)
}
