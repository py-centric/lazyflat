...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    } else if app.view_mode == crate::app::ViewMode::Logs {
        draw_logs_view(f, app, chunks[1]);
    } else {
    ...
    if app.show_help {
        let vertical = Layout::default()
            .direction(Direction::Vertical)
            .constraints([Constraint::Percentage(10), Constraint::Length(20), Constraint::Min(0)].as_ref())
            .split(f.area());
        let horizontal = Layout::default()
            .direction(Direction::Horizontal)
            .constraints([Constraint::Percentage(20), Constraint::Length(50), Constraint::Min(0)].as_ref())
            .split(vertical[1]);
        ...
        let text = vec![
            ...
            Line::from(" Enter: Launch selected"),
...
fn draw_list(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let running_ids = app.running_ids.clone();
    ...
    let mut rows: Vec<Row> = list_items
        ...
        .map(|item| {
            ...
            if running_ids.contains(&item.application_id) {
                name_text = format!("{} [RUNNING]", name_text);
...
fn draw_logs_view(f: &mut Frame, app: &App, area: Rect) {
    let app_id = app.get_selected_id().unwrap_or_default();
    let logs = app.app_logs.get(&app_id).cloned().unwrap_or_default();
    
    let text: Vec<Line> = logs.iter().map(|l| Line::from(l.as_str())).collect();
    
    let title = format!(" Logs for {} ", app_id);
    let block = Block::default().borders(Borders::ALL).title(title);
    
    let paragraph = Paragraph::new(text)
        .block(block)
        .scroll(((logs.len() as u16).saturating_sub(area.height - 2), 0));
    ...
    let status_text = match app.input_mode {
        ...
        InputMode::Normal => {
            if let Some(ref msg) = app.status_message {
                ...
                " Esc/L: Back | q: Quit ".to_string()
            } else {
                if app.current_tab == AppTab::Discover {
                    " q: Quit | ?: Help | j/k: Nav | h/l: Tabs | i: Install | r: Refresh | /: Search ".to_string()
                } else {
                    let mut base = " q: Quit | ?: Help | j/k: Nav | h/l: Tabs | r: Refresh | /: Search ".to_string();
                    if let Some(id) = app.get_selected_id() {
                        if app.is_installed(&id) {
                            base = " Ent: Run | S: Stop | L: Logs | p: Perm | x: Uninst | u: Upd | U: UpdAll ".to_string();
                        }
                    }
                    base
                }
