...
async fn main() -> Result<()> {
    ...
    let (log_tx, log_rx) = tokio::sync::mpsc::unbounded_channel::<(String, String)>();

    // Create app and run
    let mut app = App::new(log_tx);
    // Pre-load flatpak data async
    app.refresh_data().await?;

    let res = run_app(&mut terminal, &mut app, log_rx).await;
...
async fn run_app<B: Backend>(terminal: &mut Terminal<B>, app: &mut App, mut log_rx: tokio::sync::mpsc::UnboundedReceiver<(String, String)>) -> Result<()> {
    let mut last_refresh = std::time::Instant::now();
    loop {
        // Handle logs
        while let Ok((id, log)) = log_rx.try_recv() {
            let logs = app.app_logs.entry(id).or_insert_with(Vec::new);
            logs.push(log);
            if logs.len() > 500 {
                logs.remove(0);
            }
